...
class NewChatGLMForConditionalGeneration(ChatGLMForConditionalGeneration):
    def forward(
            self,
        ...
        labels = labels.to(self.transformer.output_layer.weight.device)
        return ChatGLMForConditionalGeneration.forward(
            self,
            input_ids,
            position_ids,
            attention_mask,
...
def get_model(args):
    ...
    model = NewChatGLMForConditionalGeneration.from_pretrained(
        args.pretrained_model_path, quantization_config=q_config, device_map="auto"
    )
...
def train_and_eval(
        args, model, optimizer, train_loader, valid_loader, loss_or_metric="loss"
):
    best_eval_f1 = 0
...
def main():
    ...
    train_and_eval(args, model, optimizer, train_loader, valid_loader)
